...
class Cluster {
    ...
    constructor(
        ...
        this.clusters = clustersData.map((data) =>
            createCluster(
                data,
                map,
                radiusInPixels,
        ...
        this.allSubObjectsCount =
            objects.length +
            this.clusters.reduce(
                (acc, cluster) => acc + cluster.allSubObjectsCount,
                0
            );
    }
...
export class ClusterMarkerLayer extends ObjectInMap {
    ...
    constructor(
        ...
        const props: ClusterMarkerLayerProps = {
            ...defaultClusterMarkerLayerProps,
            ...options
        };
    ...
    private _onZoomChange() {
        ...
        if (this._lastZoom >= 0 && this._lastZoom !== currentZoom) {
            ...
            const filteredWasActive = wasActive?.filter(
                (cluster) => willBeActive.indexOf(cluster) < 0
            );
    ...
    private _set(markers: ObjectInMap[]) {
        ...
        this.mainClusters = clusters.map((data) =>
            createCluster(
                data,
                this.map,
                this.radius,
                this.useAverageLocation,
                this.clusterReactElement
            )
        );
        ...
        this.mainClusters.forEach((cluster) => {
            ...
            allSubclusters.forEach((subcluster) => {
                for (
                    let zoom = subcluster.startZoom;
                    zoom <= subcluster.endZoom;
                    zoom++
                ) {
                    if (!this.clustersByZoom[zoom])
                        this.clustersByZoom[zoom] = [];
    ...
    private _splitToClusters(markers: ObjectInMap[]): ClusterData[] {
        ...
        markers.forEach((marker) => {
            ...
            for (let i = min; i <= max; i++) {
                ...
                const parentGroups = lastGroup
                    ? (lastGroup as Group).groupsDict
                    : mainGroupsDict;
                ...
                if (lastGroup && lastGroup.groups.indexOf(currentGroup) < 0)
                    lastGroup.groups.push(currentGroup);
                if (!lastGroup && mainGroups.indexOf(currentGroup) < 0)
                    mainGroups.push(currentGroup);

                if (i === max) currentGroup.objects.push(marker);
    ...
    private _simplifyGroup(group: Group): ClusterData {
        ...
        if (clusters.length > 1) cluster.clusters = clusters;

        return cluster;
    }

    private _getClusterIndexes(
        objectLocation: LocationPoint,
        zoom: number
    ): [number, number] {
